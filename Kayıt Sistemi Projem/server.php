...
if (isset($_POST['reg_user'])) {
  ...
  if ($password_1 != $password_2) {
	array_push($errors, "İki Şifre Eşleşmiyor Tekrar Deneyin");
  }
  ...
  if ($user) { // if user exists
    if ($user['username'] === $username) {
      array_push($errors, "Bu Kullanıcı Adı Bulunmaktadır");
    }

    if ($user['email'] === $email) {
      array_push($errors, "Bu E-posta Bulunmaktadır");
    }
  }
  ...
  if (count($errors) == 0) {
  	...
  	if (mysqli_num_rows($results) == 1) {
  	  ...
  	  header('location: index.php');
  		...
  		array_push($errors, "Kullanıcı Adı veya Şifre Yanlış");
  	}
  }
}
  ...
  if (count($errors) == 0) {
  	...
  	if (mysqli_num_rows($results) == 1) {
  	  ...
  	  header('location: index.php');
  		...
  		array_push($errors, "Kullancı Adı Veya Şifre Yanlış");
  	}
  }
}
